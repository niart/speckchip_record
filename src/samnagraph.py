...
class SamnaGraphBuilder:
    ...
    # looking for input device with name "input_device_name" defined in main file
    ...
    # generate a source node for an external input device
    ...
    def route_output(self, streamer_endpoint):
        ...
        self.config = self.samna_graph.sequential([samna.BasicSourceNode_ui_event(),self.streamer_node])
        return self.config
...
class SamnaConfigure:
    ...
    def davis_and_dynapcnn(self, out_layer, network):
        self.devkit_config = network.make_config(device="dynapcnndevkit:0")
...
class SamnaController:
    def __init__(self, device_name, input_device_name, visualizer_id):
        ...
        self.devkit = self.graph_builder.devkit
        self.config = None
    ...
    def setup(self):
        ...
        self.devkit = self.graph_builder.devkit
        self.buf = samna.graph.sink_from(self.devkit.get_model_source_node())
        self.graph_builder.open_input_device()
        self.graph_builder.route_input()
        self.config = self.graph_builder.route_output(self.streamer_endpoint)
